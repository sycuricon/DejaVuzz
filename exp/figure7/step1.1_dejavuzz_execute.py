...
def dejavuzz_execute_and_analysis(repo_prefix, group_prefix, command):
    ...
    command = f'make {command} TARGET_CORE=BOOM PREFIX={group_prefix} > /dev/null'
    ...
    while fuzz_thread.is_alive():
        ...
        print_time_counter += EXAMINE_INTERVAL

        if os.path.exists(leak_iter_num_path):
            with open(leak_iter_num_path, 'rt') as file:
                leak_iter_num = int(file.read())
        
        if print_time_counter >= PRINT_TIME:
            new_leak_iter_num = leak_iter_num - last_leak_iter_num
            dejavuzz_record_log(f'leak_time:{print_time_counter}\tleak_num:{leak_iter_num}\tlast_leak_num:{last_leak_iter_num}\tnew_leak_num:{new_leak_iter_num}')
            # if new_leak_iter_num == 0 and leak_time_counter >= LEAK_MAX_TIME:
            #     dejavuzz_record_log(f'fuzz {group_prefix} fails, because new leak time out')
            #     # print("leak fuzz time out")
            #     break 
            print_time_counter %= PRINT_TIME
...
def task(time_str, fuzz_config, make_target, log_dir):
    try:
        while True:
            group_prefix = time_str
            result = dejavuzz_execute_and_analysis(log_dir, f'{fuzz_config}_{group_prefix}', make_target)
            if result:
                break
...
if __name__ == "__main__":
    ...
    current_time = datetime.datetime.now()
    time_str = current_time.strftime("%Y-%m-%d-%H-%M-%S")
    repo_prefix = f'ae_djv_{time_str}'

    threads = []
    cfg = [
        ('no_cov', 'do-fuzz-no-coverage'),
        ('full', 'do-fuzz'),
    ]
    
    for fuzz_config, make_target in cfg:
        thread = threading.Thread(target=task, args=(time_str, fuzz_config, make_target, repo_prefix))
        thread.start()
