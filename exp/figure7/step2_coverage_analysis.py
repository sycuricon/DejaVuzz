...
def analysis_and_draw(specdoctor_path, dejavuzz_path):
    ...
    file_in_dir = get_spec_cov_list(specdoctor_path)
    curve = spec_get_curve(file_in_dir, 0, specdoctor_path)
    ...
    for curve in specdoctor_curve:
        plt.plot(curve, label='SpecDoctor', color="#ffbe7a")
    ...
    coverage_name = f'build/Coverage.pdf'
...
if __name__ == "__main__":
    ...
    dejavuzz_path = os.path.join(current_path, args.dejavuzz_path)
    specdoctor_path = os.path.join(current_path, args.specdoctor_path)
